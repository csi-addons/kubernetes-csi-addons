...
build: generate fmt vet ## Build manager binary.
	...
	CGO_ENABLED=0 go build -ldflags '$(LDFLAGS)' -a -o bin/csi-volume-device-exporter ./cmd/csi-volume-device-exporter
...
docker-build-exporter: container-cmd
	$(CONTAINER_CMD) build -f ./build/Containerfile.volume-device-exporter -t ${EXPORTER_IMG} .
...
deploy-exporter: ## Deploy volume-device-exporter DaemonSet and PodMonitor.
	kubectl apply -f deploy/volume-device-exporter/daemonset.yaml
	kubectl apply -f deploy/volume-device-exporter/podmonitor.yaml

.PHONY: undeploy-exporter
undeploy-exporter: ## Remove volume-device-exporter DaemonSet and PodMonitor.
	kubectl delete --ignore-not-found=$(ignore-not-found) -f deploy/volume-device-exporter/podmonitor.yaml -f deploy/volume-device-exporter/daemonset.yaml
