...
func TestVolumeGroupReplication(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		if tc.pvcFound {
			...
			pvc := &corev1.PersistentVolumeClaim{}
			err = r.Client.Get(context.TODO(), types.NamespacedName{Name: testPVC.Name, Namespace: testPVC.Namespace}, pvc)
			assert.NoError(t, err)

			vgr := &replicationv1alpha1.VolumeGroupReplication{}
			err = r.Client.Get(context.TODO(), nsKey, vgr)
			assert.NoError(t, err)
			...
			expectedOwner := fmt.Sprintf("%s/%s", volumeGroupReplication.Name, volumeGroupReplication.Namespace)
			assert.Equal(t, expectedOwner, pvc.ObjectMeta.Annotations[replicationv1alpha1.VolumeGroupReplicationNameAnnotation])
			// Check VGRContent Created
			assert.NotEmpty(t, vgr.Spec.VolumeGroupReplicationContentName)
		} else {
			// Check reconcile returned an error
			assert.Equal(t, reconcile.Result{}, res)
			assert.Error(t, err)
			...
			assert.Empty(t, vgr.Spec.VolumeGroupReplicationContentName)
		}
	}
}
