...
func (r *VolumeGroupReplicationContentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	err = r.Client.Get(ctx, types.NamespacedName{Name: instance.Spec.VolumeGroupReplicationClassName}, vgrClass)
	if err != nil {
	...
	err = r.Client.Get(ctx, namespacedObj, vgrObj)
	if err != nil {
	...
	if instance.GetDeletionTimestamp().IsZero() {
		...
		// Check if the owner VGR is marked for deletion, only then remove the finalizer from VGRContent resource
		if vgrObj.GetDeletionTimestamp().IsZero() {
			...
			return reconcile.Result{}, nil
		} else if !slices.Contains(instance.ObjectMeta.Finalizers, volumeReplicationFinalizer) {
			// Delete the volume group, if exists
			if instance.Spec.VolumeGroupReplicationHandle != "" {
				// Fetch the volume group using the group id
				tmpVG, err := volumeGroupClient.ControllerGetVolumeGroup(instance.Spec.VolumeGroupReplicationHandle, secretName, secretNamespace)
				if err != nil {
					if status.Code(err) == codes.NotFound {
						logger.Info("failed to get volume group, the volume group has already been deleted or doesn't exists")
					} else {
						logger.Error(err, "failed to get the volume group to be deleted", "Volume Group", instance.Spec.VolumeGroupReplicationHandle)
						return reconcile.Result{}, err
					}
				} else {
					// Empty the volume group before deleting the volume group, if not empty
					if len(tmpVG.VolumeGroup.GetVolumeIds()) != 0 {
						_, err := volumeGroupClient.ModifyVolumeGroupMembership(tmpVG.VolumeGroup.GetVolumeGroupId(), []string{}, secretName, secretNamespace)
						if err != nil {
							logger.Error(err, "failed to delete volumes from volume group, before deleting volume group")
							return reconcile.Result{}, err
						}
					}
					_, err := volumeGroupClient.DeleteVolumeGroup(tmpVG.VolumeGroup.GetVolumeGroupId(), secretName, secretNamespace)
					if err != nil {
						logger.Error(err, "failed to delete volume group", "Volume Group", tmpVG.VolumeGroup.GetVolumeGroupId())
						return reconcile.Result{}, err
					}
				}
			}
	...
	if instance.Spec.VolumeGroupReplicationHandle == "" {
		...
		_, err := volumeGroupClient.ModifyVolumeGroupMembership(groupID, instance.Spec.Source.VolumeHandles, secretName, secretNamespace)
		if err != nil {
...
func (r *VolumeGroupReplicationContentReconciler) annotateVolumeGroupReplicationWithVGRContentOwner(ctx context.Context, logger logr.Logger, reqOwnerName string, vgr *replicationv1alpha1.VolumeGroupReplication) error {
	...
	currentOwnerName := vgr.ObjectMeta.Annotations[replicationv1alpha1.VolumeGroupReplicationContentNameAnnotation]
	if currentOwnerName == "" {
		logger.Info("setting vgrcontent owner on VGR annotation", "Name", vgr.Name, "owner", reqOwnerName)
		vgr.ObjectMeta.Annotations[replicationv1alpha1.VolumeGroupReplicationContentNameAnnotation] = reqOwnerName
...
func (r *VolumeGroupReplicationContentReconciler) removeVGRContentOwnerFromVGRAnnotation(ctx context.Context, logger logr.Logger, vgr *replicationv1alpha1.VolumeGroupReplication) error {
	if _, ok := vgr.ObjectMeta.Annotations[replicationv1alpha1.VolumeGroupReplicationContentNameAnnotation]; ok {
