...
func (r *VolumeGroupReplicationContentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if instance.GetDeletionTimestamp().IsZero() {
		...
		if err = r.addFinalizerToVGRContent(logger, instance); err != nil {
		...
		if vgrObj.GetDeletionTimestamp().IsZero() {
			...
			// Delete the volume group, if created
			if instance.Spec.VolumeGroupReplicationHandle != "" {
				_, err := volumeGroupClient.DeleteVolumeGroup(instance.Spec.VolumeGroupReplicationHandle, secretName, secretNamespace)
				if err != nil {
			...
			if err = r.removeFinalizerFromVGRContent(logger, instance); err != nil {
	...
	if instance.Spec.VolumeGroupReplicationHandle == "" {
		groupName := fmt.Sprintf("volumegroup-%s", instance.UID)
		...
		instance.Spec.VolumeGroupReplicationHandle = resp.GetVolumeGroupId()
	...
	for _, pv := range pvList.Items {
		if slices.ContainsFunc(instance.Spec.Source.VolumeHandles, func(handle string) bool {
			return pv.Spec.CSI.VolumeHandle == handle
		}) {
