...
func annotatePVCWithOwner(client client.Client, ctx context.Context, logger logr.Logger, reqOwnerName string,
	...
	if pvc.ObjectMeta.Annotations[replicationv1alpha1.VolumeReplicationNameAnnotation] != "" &&
		...
		return fmt.Errorf("PVC (%q/%q) can't be owned by both VolumeReplication and VolumeGroupReplication", pvc.Name, pvc.Namespace)
	}
	...
	if currentOwnerName == "" {
		...
		if err != nil {
			...
			return fmt.Errorf("failed to update PVC (%q/%q) annotation for replication: %w", pvc.Name, pvc.Namespace, err)
		}
	...
	if currentOwnerName != reqOwnerName {
		...
		return fmt.Errorf("PVC (%q/%q) not owned by correct VolumeReplication/VolumeGroupReplication %q",
			pvc.Name, pvc.Namespace, reqOwnerName)
	}
	...
	if _, ok := pvc.ObjectMeta.Annotations[pvcAnnotation]; ok {
		...
		if err := client.Update(ctx, pvc); err != nil {
			return fmt.Errorf("failed to remove annotation %q from PersistentVolumeClaim "+
				"(%q/%q) %w",
				pvcAnnotation, pvc.Name, pvc.Namespace, err)
		}
	}
