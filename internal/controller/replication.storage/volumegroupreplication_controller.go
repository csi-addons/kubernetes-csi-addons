...
func (r *VolumeGroupReplicationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if instance.GetDeletionTimestamp().IsZero() {
		...
		pvcList, pvHandlesList, labelSelector, err := r.getMatchingPVCsFromSource(instance, vgrClassObj)
		if err != nil {
		...
		// Remove volumereplication owner annotation from VGR
		err = r.removeOwnerFromVGRAnnotation(instance)
		if err != nil {
			return reconcile.Result{}, err
		}
...
func (r *VolumeGroupReplicationReconciler) getMatchingPVCsFromSource(instance *replicationv1alpha1.VolumeGroupReplication,
	vgrClass *replicationv1alpha1.VolumeGroupReplicationClass) ([]corev1.PersistentVolumeClaim, []string, string, error) {
	...
	err := r.Client.List(r.ctx, &pvcList, opts...)
	if err != nil {
		r.log.Error(err, "failed to list pvcs with the given selectors", "Selector", newSelector.String())
		return nil, nil, "", err
	}

	// Filter out PVCs which are marked for deletion
	filteredPVCList := []corev1.PersistentVolumeClaim{}
	for _, pvc := range pvcList.Items {
		if pvc.GetDeletionTimestamp().IsZero() {
			filteredPVCList = append(filteredPVCList, pvc)
		}
	}

	pvHandlesList := []string{}
	for _, pvc := range filteredPVCList {
		pvName := pvc.Spec.VolumeName
		pv := &corev1.PersistentVolume{}
		err := r.Client.Get(r.ctx, types.NamespacedName{Name: pvName}, pv)
		if err != nil {
	...
	return filteredPVCList, pvHandlesList, newSelector.String(), nil
}
...
func (r *VolumeGroupReplicationReconciler) updateFinalizerAndAnnotationOnPVCs(vgr *replicationv1alpha1.VolumeGroupReplication,
	...
	for name := range oldPVCs {
		if _, exists := currentPVCs[name]; !exists {
			...
			err := r.Client.Get(r.ctx, types.NamespacedName{Name: name, Namespace: vgr.Namespace}, &tmpPVC)
			if err != nil {
	...
	for _, pvc := range toAddPVCs {
		reqOwner := fmt.Sprintf("%s/%s", vgr.Name, vgr.Namespace)
...
func (r *VolumeGroupReplicationReconciler) cleanupGroupPVC(vgr *replicationv1alpha1.VolumeGroupReplication) error {
	for _, pvcRef := range vgr.Status.PersistentVolumeClaimsRefList {
		...
		err := r.Client.Get(r.ctx, types.NamespacedName{Name: pvcRef.Name, Namespace: vgr.Namespace}, pvc)
		if err != nil {
...
func (r *VolumeGroupReplicationReconciler) createOrUpdateVolumeGroupReplicationContentCR(vgr *replicationv1alpha1.VolumeGroupReplication,
	...
	vgrRef := fmt.Sprintf("%s/%s", vgr.Name, vgr.Namespace)
...
func (r *VolumeGroupReplicationReconciler) cleanupVGRContent(vgr *replicationv1alpha1.VolumeGroupReplication,
	...
	err := r.Client.Get(r.ctx, req, vgrContentObj)
	if err != nil {
		if !apierrors.IsNotFound(err) {
		...
		err = r.Client.Delete(r.ctx, vgrContentObj)
		if err != nil {
...
func (r *VolumeGroupReplicationReconciler) removeOwnerFromVGRAnnotation(vgr *replicationv1alpha1.VolumeGroupReplication) error {
	if _, ok := vgr.ObjectMeta.Annotations[replicationv1alpha1.VolumeReplicationNameAnnotation]; ok {
		r.log.Info("removing owner annotation from VolumeGroupReplication object", "Annotation", replicationv1alpha1.VolumeReplicationNameAnnotation)
		delete(vgr.ObjectMeta.Annotations, replicationv1alpha1.VolumeReplicationNameAnnotation)
		if err := r.Client.Update(r.ctx, vgr); err != nil {
