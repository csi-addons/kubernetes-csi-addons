...
func (r *VolumeGroupReplicationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if instance.GetDeletionTimestamp().IsZero() {
		...
		pvcRefList, err := r.updateFinalizerAndAnnotationOnPVCs(instance, pvcList, logger)
		if err != nil {
			_ = r.setGroupReplicationFailure(instance, logger, err)
			return reconcile.Result{}, err
		}
		...
		logger.Info("volumeGroupReplication object is terminated, skipping reconciliation")
		return reconcile.Result{}, nil
	}
...
func (r *VolumeGroupReplicationReconciler) updateFinalizerAndAnnotationOnPVCs(vgr *replicationv1alpha1.VolumeGroupReplication, pvcList corev1.PersistentVolumeClaimList,
	logger logr.Logger) ([]corev1.LocalObjectReference, error) {
	...
	currentPVCs := make(map[string]struct{}, len(pvcList.Items))
	...
	for name := range oldPVCs {
		if _, exists := currentPVCs[name]; !exists {
			...
			err := r.Client.Get(context.TODO(), types.NamespacedName{Name: name}, &tmpPVC)
			if err != nil {
				logger.Error(err, "failed to get pvc", "PVC Name", name)
				return nil, err
			}
	...
	for _, pvc := range toAddPVCs {
		...
		if err = AddFinalizerToPVC(r.Client, logger, &pvc, vgrReplicationFinalizer); err != nil {
			logger.Error(err, "Failed to add VGR finalizer on PVC")
			return nil, err
		}
	}
	...
	for _, pvc := range toRemovePVCs {
		err := RemoveOwnerFromPVCAnnotation(r.Client, logger, &pvc, replicationv1alpha1.VolumeGroupReplicationNameAnnotation)
		if err != nil {
			logger.Error(err, "Failed to remove VGR owner annotation from PVC")
			return nil, err
		}

		if err = RemoveFinalizerFromPVC(r.Client, logger, &pvc, vgrReplicationFinalizer); err != nil {
			logger.Error(err, "Failed to remove VGR finalizer from PVC")
			return nil, err
		}
	}
...
func (r *VolumeGroupReplicationReconciler) createOrUpdateVolumeGroupReplicationContentCR(vgr *replicationv1alpha1.VolumeGroupReplication,
	...
	_, err := controllerutil.CreateOrUpdate(context.TODO(), r.Client, vgrContentObj, func() error {
		if vgr.Spec.VolumeGroupReplicationContentName != "" && vgrContentObj.CreationTimestamp.IsZero() {
			return fmt.Errorf("dependent VGRContent resource is not yet created, waiting for it to be created")
		}
