...
var (
	...
	rsCronJobExcludeAnnotation      = "reclaimspace." + csiaddonsv1alpha1.GroupVersion.Group + "/exclude"
...
func (r *PersistentVolumeClaimReconciler) processReclaimSpace(
	...
	if rsCronJob != nil {
		*logger = logger.WithValues("ReclaimSpaceCronJobName", rsCronJob.Name)
		if _, ok := rsCronJob.GetAnnotations()[rsCronJobExcludeAnnotation]; ok {
			logger.Info("ReclaimSpaceCronJob has exclude annotation set, exiting reconcile")
			return ctrl.Result{}, nil
		}
	}
