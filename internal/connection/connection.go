...
// more than `IDLE_TIMEOUT`. If so, the connection is closed.
func (c *Connection) startIdleTimer(ctx context.Context) {
	...
	for {
		...
		case <-ticker.C:
			...
			isIdle := time.Since(lastAccess) > IDLE_TIMEOUT
...
// ensureConnected re-eastablishes a connection if it is in a disconnected
// state. An error is returned if it fails to renew the connection.
