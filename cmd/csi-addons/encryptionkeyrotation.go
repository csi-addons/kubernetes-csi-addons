...
func (ekr *EncryptionKeyRotation) Init(c *command) error {
	if ekr.persistentVolume == "" {
		return fmt.Errorf("persistent volume name is not set")
	}
...
func (ekr *EncryptionKeyRotation) Execute() error {
	...
	ekrRes, err := ekrs.KeyRotate(context.TODO(), ekrReq)
	if err != nil {
		return err
	}

	if ekrRes == nil {
		return fmt.Errorf("failed to rotate enc key for pv: %s", ekr.persistentVolume)
	}
