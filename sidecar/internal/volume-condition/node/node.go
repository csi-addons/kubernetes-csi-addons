...
func NewNode(ctx context.Context, clientset *kubernetes.Clientset, nodename string) (Node, error) {
	// verify that my local Node exists, will be fetched again in .ListVolumes()
...
func (n *node) ListVolumes(ctx context.Context) ([]volume.Volume, error) {
...
func newAttachedCSIVolume(name corev1.UniqueVolumeName) (volume.Volume, error) {
	const (
	...
	volDriver := strings.Split(driverVolume, driverVolumeSeparator)[0]
	volHandle := strings.Split(driverVolume, driverVolumeSeparator)[1]

	return volume.NewCSIVolume(volDriver, volHandle), nil
}
