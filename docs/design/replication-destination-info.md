...
`VolumeGroupReplicationStatus` is **not modified**. It retains the
existing `PersistentVolumeClaimsRefList []corev1.LocalObjectReference`
field (PVC names only). All destination volume handle mappings and the
destination volume group ID are stored exclusively in
`VolumeGroupReplicationContentStatus` (see Section 9a). This preserves
the architectural layering where VGR contains PVC-level information and
VGRContent contains CSI/SP-level details.

> **Note:** DR orchestrators read PVC names from VGR.Status and
> destination volume handle mappings from
> VGRContent.Status.PersistentVolumeMappingList. Each entry contains the
> PV name, source volume handle, and destination volume handle — no
> cross-referencing needed. The `DestinationInfoAvailable` condition on
> VGR signals when VGRContent.Status destination fields are complete
> and ready to use.
...
Add a new enriched PV status type and update
...
> **Note:** The deprecated `PersistentVolumeRefList` continues to be
> populated for backward compatibility. New consumers SHOULD use
> `PersistentVolumeMappingList` which provides the complete picture:
> PV name, source volume handle, and destination volume handle in a
> single entry per PV.
...
VGR reconciles when VGRContent destination info is updated. This triggers
the VGR controller to check VGRContent.Status destination fields and
update the `DestinationInfoAvailable` condition on VGR:
...
VGR controller reconciles (triggered by VGRContent status watch):
  ...
  2. Validates map covers all source volume handles
  3. If complete: sets DestinationInfoAvailable=True on VGR
  4. If incomplete: keeps DestinationInfoAvailable=False, requeues
```

> **Note:** The VGR controller does not copy destination data to VGR.Status.
> Destination volume handle mappings and the destination volume group ID
> remain exclusively in VGRContent.Status. The VGR controller only validates
> completeness and sets the `DestinationInfoAvailable` condition on VGR as a
> signal for DR orchestrators.
...
// PersistentVolumeClaimsRefList remains []corev1.LocalObjectReference
// (PVC names only). Destination details stay in VGRContent.Status.
...
if r.supportsGetReplicationDestinationInfo(ctx, driverName) {
    ...
    if vgDest := destInfo.GetReplicationDestination().GetVolumegroup(); vgDest != nil {
        ...
        // Build PersistentVolumeMappingList with destination handles.
        // The SP returns a map of source handle -> destination handle.
        destVolumeIDs := vgDest.GetVolumeIds()
        pvMappings := make([]PersistentVolumeMapping, 0, len(vgrcInstance.Spec.Source.VolumeHandles))
        for _, srcHandle := range vgrcInstance.Spec.Source.VolumeHandles {
            pv := getPVByVolumeHandle(srcHandle)
            pvMapping := PersistentVolumeMapping{
                Name:         pv.Name,
                VolumeHandle: srcHandle,
            }
            if destHandle, ok := destVolumeIDs[srcHandle]; ok {
                pvMapping.DestinationVolumeHandle = destHandle
            }
            pvMappings = append(pvMappings, pvMapping)
        }
        vgrcInstance.Status.PersistentVolumeMappingList = pvMappings

        // Continue populating deprecated field for backward compatibility
        pvRefList := make([]corev1.LocalObjectReference, 0, len(pvMappings))
        for _, pvMapping := range pvMappings {
            pvRefList = append(pvRefList, corev1.LocalObjectReference{Name: pvMapping.Name})
        }
        vgrcInstance.Status.PersistentVolumeRefList = pvRefList

        // Update VGRContent status only — VGR watches for status changes
        // and will set DestinationInfoAvailable condition accordingly
    }
}
```

#### 9f. VGR Controller: Validate and Signal Readiness

On each reconcile, the VGR controller reads the VGRContent status and
validates that the destination map is complete. If complete, it sets the
`DestinationInfoAvailable` condition to `True` on VGR. The destination
data itself (volume handle mappings and destination group ID) remains
exclusively in VGRContent.Status — DR orchestrators read it from there:
...
if r.supportsGetReplicationDestinationInfo(ctx, driverName) &&
    ...
    if len(vgrc.Status.PersistentVolumeMappingList) > 0 {
        // Validate: every PV entry must have a destination handle
        allCovered := true
        ...
        if allCovered {
            // Signal that VGRContent.Status has complete destination info
            setDestinationInfoAvailableCondition(&instance.Status.Conditions,
                instance.Generation, volumeGroupReplicationDataSource)
        }
...
VGR validates map covers all source volume handles
VGR sets DestinationInfoAvailable=True (data remains in VGRContent.Status)
DestinationInfoAvailable: True (Reason: DestinationInfoUpdated)
...
#### Step 2: Read Destination Info from VGRContent Status on Primary Cluster

Once replication is established and `DestinationInfoAvailable` is `True`
on the VGR, the DR orchestrator reads destination details from
VGRContent.Status:

VGR status (PVC names and readiness signal):

```yaml
# VolumeGroupReplication status
status:
  state: Primary
...
VGRContent status (destination volume handle mappings):

```yaml
# VolumeGroupReplicationContent status
status:
  ...
  # deprecated: persistentVolumeRefList is still populated for
...
2. Only use destination IDs when the condition is `True`.
3. Read `destinationVolumeGroupID` and `persistentVolumeMappingList` from
   VGRContent.Status. Each `PersistentVolumeMapping` entry contains the
   PV name, source volume handle, and destination volume handle — no
   need to cross-reference maps or look up PV objects.
4. Store the mapping in its own state (e.g., a DRPlacementControl or
   similar CR) so it is available during failover.
...
5. The DR orchestrator reads the updated `persistentVolumeMappingList`
   from VGRContent.Status, which now includes entries for the newly
   added volumes (and omits entries for removed volumes).
...
```console
  ...
  VGR reconciles (triggered by VGRContent status watch):
    ...
    - Sets DestinationInfoAvailable=True on VGR
  DestinationInfoAvailable: True (DestinationInfoUpdated)
  VGRContent.Status.PersistentVolumeMappingList now includes:
    pv-data-4: volumeHandle=src-vol-004, destinationVolumeHandle=dest-vol-004
      |
      v
  DR orchestrator reads updated VGRContent.Status, stores mapping
```
...
For each entry in `VGRContent.Status.PersistentVolumeMappingList`, the DR
orchestrator reads the `destinationVolumeHandle` directly:

```yaml
# Each PersistentVolumeMapping entry has all the info needed:
#   pv-data-1: volumeHandle=src-vol-001, destinationVolumeHandle=dest-vol-001
#   pv-data-2: volumeHandle=src-vol-002, destinationVolumeHandle=dest-vol-002
#   pv-data-3: volumeHandle=src-vol-003, destinationVolumeHandle=dest-vol-003
...
VGR (vgr-app-data)                   PV (dest-vol-001) <-- Step 4a
  ...
  persistentVolumeClaimsRefList:     PV (dest-vol-003)
    - pvc-data-1                         |
    - pvc-data-2                     PVC (pvc-data-1)  <-- Step 4b
    - pvc-data-3                     PVC (pvc-data-2)
  conditions:                        PVC (pvc-data-3)
    DestinationInfoAvailable: True       |
        |
VGRContent (primary)                 VGRContent         <-- Step 4c
  status:                              handle: "dest-group-0001"
    destinationVolumeGroupID:          volumeHandles:
      "dest-group-0001"                  - dest-vol-001
    persistentVolumeMappingList:            - dest-vol-002
      pv-data-1:                         - dest-vol-003
        src: src-vol-001                 |
        dest: dest-vol-001
      pv-data-2:
        src: src-vol-002
      ...
      pv-data-3:
        ...
        | DR reads                     state: primary
        | destination                  contentName: vgrcontent-failover
        | info from                      |
        | VGRContent.Status          VR (auto-created by VGR controller)
        +------>                       state: primary
```
...
On the secondary cluster (now acting as primary), read the VGRContent
status:

```yaml
# VGRContent on secondary cluster (current primary)
status:
  destinationVolumeGroupID: "src-group-0001" # points back to original primary
  persistentVolumeMappingList:
    - name: pv-data-1
      volumeHandle: "dest-vol-001" # source on this cluster
...
`destinationVolumeHandle` from each `PersistentVolumeMapping` entry to
resolve the volume IDs on the original primary.
...
| Store mapping (VR)      | Read `destinationVolumeID` from VR.Status                      | N/A                                          |
| Store mapping (VGR)     | Read `persistentVolumeMappingList` from VGRContent.Status      | N/A                                          |
...
2. **`destinationVolumeID` field is empty** on VR status (or
   `destinationVolumeGroupID` is empty and `persistentVolumeMappingList`
   entries have no `destinationVolumeHandle` on VGRContent status).
...
status:
  ...
  # persistentVolumeMappingList is absent (or entries have no destinationVolumeHandle)
  persistentVolumeRefList:
    - name: pv-data-1
    - name: pv-data-2
    - name: pv-data-3
...
1. **Never assume source ID equals destination ID when destination info is
   available.** When `destinationVolumeID` (VR) or
   `persistentVolumeMappingList[].destinationVolumeHandle` (VGRContent) fields
...
3. **For volume groups, read destination mappings from VGRContent.Status.**
   Read `persistentVolumeMappingList` from VGRContent.Status — each entry
   contains the PV name, source volume handle, and destination volume
   handle directly. No need to cross-reference maps or look up PV objects.
...
4. **Implement a unified volume ID resolution function.** The DR
   ...
   ```go
   // For individual VolumeReplication
   ...
   // For VolumeGroupReplication — iterate PersistentVolumeMappingList
   func resolveGroupVolumeID(vgrcStatus VolumeGroupReplicationContentStatus, sourceVolumeID string) string {
       for _, pvMapping := range vgrcStatus.PersistentVolumeMappingList {
           if pvMapping.VolumeHandle == sourceVolumeID && pvMapping.DestinationVolumeHandle != "" {
               return pvMapping.DestinationVolumeHandle
           }
       }
       // Capability not supported or not yet available: source ID equals destination ID
       return sourceVolumeID
   }
   ```
